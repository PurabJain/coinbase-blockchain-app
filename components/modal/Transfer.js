...
const Transfer = ({
  ...
  useEffect(() => {
    const activeToken = thirdWebTokens.find(
      (token) => token.address === selectedTokens.contractAddress
    );
  ...
  useEffect(() => {
    ...
    const builder = imageUrlBuilder(client)
      .projectId("2zdx3eow")
      .dataset("production");
    const url = builder.image(selectedTokens?.logo?.asset?._ref).url();
  ...
  return (
    <Wrapper>
      ...
      <Row>
        <BalanceTitle>{selectedTokens.symbol} Balance</BalanceTitle>
        <Balance>
          {balance} {selectedTokens.symbol}
        </Balance>
      </Row>
    </Wrapper>
  );
